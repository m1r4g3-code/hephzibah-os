...
**Open questions, not yet resolved:** whether the pipeline auto-proceeds from Generate Videos → Edit Videos, or whether a video-review gate (Scene Approval has a `SERAMAN | Send Video For Review` node whose trigger source wasn't traced) sits in between requiring another manual push. Will be resolved empirically once generation completes rather than assumed — do not tell Giovanni "video is finishing itself" until that's actually confirmed.
