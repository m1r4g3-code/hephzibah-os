...
**Standing follow-up flagged, not yet built:** `SERAMAN | Interpret Scene Correction` needs the same product-structure safety rules (axis classification, SINGLE-PIECE SOFT COMPONENTS, TWO-HAND LIFT-AND-TURN) that `SERAMAN | Generate Script` already has. Right now, any correction round on a soft single-piece product (this K9 line, or similar future products) can silently reintroduce a rigid-headphone hallucination with no guardrail catching it, and each further correction round on an already-corrupted row just preserves the corruption rather than fixing it. This is a real, repeatable risk, not specific to this one job.
